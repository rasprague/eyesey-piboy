...
function draw()
    ...
    ----------------------- Color stuff
--    colorPickHsb( knob4, fg )                   -- foreground color 
    ...
    margin = (knob1 * 1.95) + 0.05              -- set the amount of space between the rectangles, min of 0.05, max 1
    ...
    -- to add animation from incoming sound we will use the audio buffer named inL
    ...
    for i = 0, 9 do                             -- the variable i starts at 0, and goes to 9, 10 loops
        ...
        for j = 0, 9 do                         -- the j loop will be the y positions so we draw the rectangles going down
            ...
            zPos = (audio * 8 )
            audioR = math.abs(inR[ count+1 ] )

            colorPickHsb( knob2+audioR, fg )
            of.setColor( fg )
            --myRect = of.Rectangle(xPos, yPos, marginW, marginH)
            rectPos = glm.vec3( xPos, yPos, zPos )
            myRect = of.Rectangle(rectPos, marginW, marginH)
            --myRect:setFromCenter(rectPos, marginW, marginH)
            -- xPos,yPos,zPos 
            myRect:scaleFromCenter(zPos)
            of.drawRectangle(myRect) -- x,y,z position, width, height in pixels
            --
