...
function colorCycle(startHue, name) -- startHue is 0-255
    hue = (startHue + 1) % 255  
    start = hue 
    name:setHsb(hue, 255, 255 )
end

function drawHeart(xloc,yloc,zloc,size)
  myLine = of.Polyline()
  i = 0;
  while (i < 2*math.pi) do -- make a heart
      r = (2-2*math.sin(i) + math.sin(i)*math.sqrt(math.abs(math.cos(i))) / (math.sin(i)+1.4)) * - size
      x = xloc + math.cos(i) * r
      y = yloc + math.sin(i) * r

      myLine:addVertex(glm.vec3(x,y,zloc))

      i = i + 0.005*math.pi/2*0.5
  end
  myLine:close();
end
